 ...
 * Creacion: 13/08/2017
 * Descripcion: Clase para timbrado
 */
...
class Certificados{
	...
	function getSelloCer($nombreCer,$nombreKeyPem){
	  $salidaKeyPem = openssl_pkey_get_private(file_get_contents($nombreKeyPem));

	  $XDOC = new DomDocument();
	  $XSL = new DOMDocument();
	  	
	  $load = $GLOBALS['CONFIG']['URL_NUCLEO'] . "utilidades/xslt33/cadenaoriginal_3_3.xslt";
	  $XSL->load($load);
	  ...
	  openssl_sign($cadenaOriginal, $sig, $salidaKeyPem, OPENSSL_ALGO_SHA256);
	  
	  return base64_encode($sig);
	}

	function getCertificado($nombreCer){
		return str_replace(array('\n', '\r'), '', base64_encode(file_get_contents($nombreCer)));
	}
}
